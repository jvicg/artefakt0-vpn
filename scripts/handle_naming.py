...
# e.g: if the file is needed by the role 'common' it will  be moved to roles/common/files
...
def handle_naming():
    ...
    for i in range(len(files)):
        try:
            if files[i].startswith('common_'): # Ansible role's files will have the prefix '<role-name>_' 
